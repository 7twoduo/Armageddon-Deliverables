...
locals {
  ...
  lab1c_bonusA_example_fqdn = "${var.app_subdomain}.${var.domain_name}"
}
...
resource "aws_security_group" "lab1c_bonusA_example_alb_sg01" {
  name        = "${var.project_name}-alb-sg01"
  description = "ALB security group"
  vpc_id      = aws_vpc.lab1c_bonusA_example_vpc01.id

    # Inbound from Internet to ALB
  
  # Outbound from ALB -> private EC2 targets on target group port (80)
  egress {
    description   = "ALB to EC2 targets on port 80"
    from_port     = 80
    to_port       = 80
    protocol      = "tcp"
    security_groups = [aws_security_group.lab1c_bonusA_example_ec2_sg01.id]
  }
...
resource "aws_security_group_rule" "lab1c_bonusA_example_ec2_ingress_from_alb01" {
  type                     = "ingress"
  security_group_id        = aws_security_group.lab1c_bonusA_example_ec2_sg01.id
  from_port                = 80
  to_port                  = 80
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.lab1c_bonusA_example_alb_sg01.id
...
resource "aws_lb" "lab1c_bonusA_example_alb01" {
  name               = "${var.project_name}-alb01"
  load_balancer_type = "application"
  internal           = false

  security_groups = [aws_security_group.lab1c_bonusA_example_alb_sg01.id]
  subnets         = aws_subnet.lab1c_bonusA_example_public_subnets[*].id
...
resource "aws_lb_target_group" "lab1c_bonusA_example_tg01" {
...
resource "aws_lb_target_group_attachment" "lab1c_bonusA_example_tg_attach01" {
  target_group_arn = aws_lb_target_group.lab1c_bonusA_example_tg01.arn
  target_id        = aws_instance.lab1c_bonusA_example_ec201.id
  port             = 80
...
############################################
# ACM Certificate (TLS) for app.lab1c_bonusA_example-growl.com
############################################

# Explanation: TLS is the diplomatic passport — browsers trust you, and Chewbacca stops growling at plaintext.
resource "aws_acm_certificate" "lab1c_bonusA_example_acm_cert01" {
  domain_name       = local.lab1c_bonusA_example_fqdn
...
# resource "aws_route53_record" "lab1c_bonusA_example_acm_validation" { ... }

# Explanation: Once validated, ACM becomes the “green checkmark” — until then, ALB HTTPS won’t work.
resource "aws_acm_certificate_validation" "lab1c_bonusA_example_acm_validation01" {
  certificate_arn = aws_acm_certificate.lab1c_bonusA_example_acm_cert01.arn

  # TODO: if using DNS validation, students must pass validation_record_fqdns
  # validation_record_fqdns = [aws_route53_record.lab1c_bonusA_example_acm_validation.fqdn]
}
...
resource "aws_lb_listener" "lab1c_bonusA_example_http_listener01" {
  load_balancer_arn = aws_lb.lab1c_bonusA_example_alb01.arn
  port              = 80
  protocol          = "HTTP"
...
resource "aws_lb_listener" "lab1c_bonusA_example_https_listener01" {
  load_balancer_arn = aws_lb.lab1c_bonusA_example_alb01.arn
  port              = 443
  protocol          = "HTTPS"
  ssl_policy        = "ELBSecurityPolicy-TLS13-1-2-2021-06"

  certificate_arn = "arn:aws:acm:us-east-2:329327479095:certificate/c2b80cdc-4e64-4cb6-abcc-028b27db2874"
...
resource "aws_wafv2_web_acl" "lab1c_bonusA_example_waf01" {
...
resource "aws_wafv2_web_acl_association" "lab1c_bonusA_example_waf_assoc01" {
  count = var.enable_waf ? 1 : 0

  resource_arn = aws_lb.lab1c_bonusA_example_alb01.arn
  web_acl_arn  = aws_wafv2_web_acl.lab1c_bonusA_example_waf01[0].arn
}
...
resource "aws_cloudwatch_metric_alarm" "lab1c_bonusA_example_alb_5xx_alarm01" {
  ...
  dimensions = {
    LoadBalancer = aws_lb.lab1c_bonusA_example_alb01.arn_suffix
  }
...
resource "aws_cloudwatch_dashboard" "lab1c_bonusA_example_dashboard01" {
    ...
    widgets = [
      {
        ...
        properties = {
          metrics = [
            [ "AWS/ApplicationELB", "RequestCount", "LoadBalancer", aws_lb.lab1c_bonusA_example_alb01.arn_suffix ],
            [ ".", "HTTPCode_ELB_5XX_Count", ".", aws_lb.lab1c_bonusA_example_alb01.arn_suffix ]
          ]
          period = 300
          stat   = "Sum"
      ...
      },
      {
        ...
        properties = {
          metrics = [
            [ "AWS/ApplicationELB", "TargetResponseTime", "LoadBalancer", aws_lb.lab1c_bonusA_example_alb01.arn_suffix ]
          ]
          period = 300
          stat   = "Average"
          region = var.aws_region
